...
class StudentHocLuc extends Component {
    ...
    render() {
        ...
        var hocki = score.map((s,index)=>{
            return (
                <li className="nav-item">
                    <a key={index}className={index===0?"nav-link active text-dark":"nav-link text-dark"} id={index+"-tab"} data-toggle="tab" href={"#"+index} role="tab" aria-controls={index} aria-selected={index===0? "true" : "false" }><strong>{s[0].hocki}</strong></a>
                </li>
            )
        })
        ...
        return (
            <div className="container my-3" style={{'borderRadius':'10px', 'padding':'10px','boxShadow':'1px 2px 5px grey'}}>
